...
class VoiceConfig(Base):
    ...
    wake_word_keywords: list[str] =  Field(default_factory=list)  # list of wake words
    wake_word_keyword_paths: list[str] =  Field(default_factory=list)  # path to custom wakeword .ppn file 
    wake_word_sensitivities: list[str] =  Field(default_factory=list) # set higher for longer distances to microphone
...
class VoiceChannel(BaseChannel):
    ...
    async def start(self) -> None:
        ...
        try:
            ...
            if len(keywords) < 0 or len(keywords[0].strip()) < 1:
                keywords = None
            
            keyword_paths: Optional[list[str]] = None
            if not keywords:
                keyword_paths = self.config.wake_word_keyword_paths
                if len(keyword_paths) < 0 or len(keyword_paths[0].strip()) < 1:
                    keyword_paths = None
